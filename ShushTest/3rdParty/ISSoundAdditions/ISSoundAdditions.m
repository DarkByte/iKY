...
+ (void)setSystemOutputVolume:(float)theVolume
{
...
+ (void)increaseSystemOutputVolumeBy:(float)amount {
    [self setSystemOutputVolume:self.systemOutputVolume+amount];
}

+ (void)decreaseSystemOutputVolumeBy:(float)amount {
    [self setSystemOutputVolume:self.systemOutputVolume-amount];
}

#pragma mark - Mute

+ (void)muteOutput:(Boolean)m
{
	AudioDeviceID				defaultDevID = kAudioObjectUnknown;
	AudioObjectPropertyAddress	theAddress;
